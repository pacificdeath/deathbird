...
static int setup_texture_scrollers_for_current_level(State *state) {
    ...
    case AREA_DEEP_FOREST: {
        tex_scrollers[0].tex = TEX_AREA_RED_SKY;
        tex_scrollers[0].horizontal_textures = 4;
        tex_scrollers[0].horizontal_speed = -0.03f;
        tex_scrollers[0].vertical_textures = 2;
        tex_scrollers[0].vertical_speed = 0.0f;
        ...
        tex_scrollers[1].tex = TEX_AREA_BIG_TREES_3;
        tex_scrollers[1].horizontal_textures = 4;
        tex_scrollers[1].horizontal_speed = 0.05f;
        tex_scrollers[1].vertical_textures = 1;
        tex_scrollers[1].vertical_speed = 0.0f;
        tex_scrollers[1].opacity = 255;

        tex_scrollers[2].tex = TEX_AREA_BIG_TREES_2;
        tex_scrollers[2].horizontal_textures = 4;
        tex_scrollers[2].horizontal_speed = 0.25f;
        tex_scrollers[2].vertical_textures = 1;
        tex_scrollers[2].vertical_speed = 0.0f;
        tex_scrollers[2].opacity = 255;

        tex_scrollers[3].tex = TEX_AREA_BIG_TREES_1;
        tex_scrollers[3].horizontal_textures = 4;
        tex_scrollers[3].horizontal_speed = 0.75f;
        tex_scrollers[3].vertical_textures = 1;
        tex_scrollers[3].vertical_speed = 0.0f;
...
static void generate_next_level_data(State *state) {
    uint next_level_idx = state->level_idx + 1;

    if (next_level_idx >= LEVELS_BEFORE_MIN_FREQUENCY) {
        state->next_level_data.bird_frequency = MIN_BIRD_FREQUENCY;
    } else {
        ...
        state->next_level_data.bird_frequency = MAX_BIRD_FREQUENCY - bird_frequency_loss;
    }
}

void level_setup_first(State *state) {
    state->level_idx = 0;
    state->next_level_data.area = AREA_FOREST;
    generate_next_level_data(state);
}
...
void level_setup_next(State *state) {
    ...
    switch (state->level_current_data.area) {
    default:
    case AREA_NONE: area = AREA_FOREST; break;
    case AREA_FOREST: area = AREA_MEADOWS; break;
    case AREA_MEADOWS: area = AREA_MOUNTAINS; break;
    case AREA_MOUNTAINS: area = AREA_FOREST; break;
    }
...
void level_render(State *state) {
    ...
    int font_size = (MENU_FONT_SIZE / state->menu.font.baseSize) * state->scale_multiplier;
    Vector2 level_idx_text_dimensions = MeasureTextEx(state->menu.font, buffer, font_size, 0);
    ...
    DrawTextPro(state->menu.font, buffer, level_idx_position, (Vector2) { 0, 0 }, 0, font_size, 0, LEVEL_IDX_TEXT_COLOR);
    ...
    if (state->bird_multiplier_timer > 0.0f) {
        ...
        DrawTextPro(state->menu.font, buffer, multiplier_text_position, (Vector2) { 0, 0 }, 0, font_size, 0, MULTIPLIER_TEXT_COLOR);
    }
}
